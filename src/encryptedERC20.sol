...
interface IEDarkPool {
    function tokenContractAddress() external returns(address);
...
contract EncryptedERC20 is Reencrypt, Ownable2Step {
    ...
    IEDarkPool immutable private poolAddress;
    TokenPool immutable private nft;
    ...
    constructor(string memory name_, string memory symbol_, address _poolAddress) Ownable(msg.sender) {
        _name = name_;
        _symbol = symbol_;
        poolAddress = IEDarkPool(_poolAddress);
        nft = new TokenPool("Dark Hole", "DH", address(poolAddress), 1_000, 100000000000000 wei);
    }
    ...
    function getTokenContractAddress() public view returns(address) {
        return nft.tokenContractAddress();
    }
